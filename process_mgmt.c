...
#include <stdlib.h>
#include <string.h>
#include <process.h> //For process management (execvp, kill, etc.)
#include <Windows.h>
...
const int mQUANTUM = 1;
const int mTotalMem = TOTAL_MEM - RESERVED_MEM;
int mDispatcher_timer;
char *mProcessName[] = {"./process", NULL};
int status = 0;
...
void ReqRes_free(pcbptr process) {
...
void start_dispatcher() {
    do {
        ...
        while(mJobs.front != NULL &&
              memoryCheck(mJobs.front->info[memory_alloc]) &&
                ReqRes_check(mJobs.front->info[num_printers], mJobs.front->info[num_scanners],
            ...
            allocatedMem = AllocMemory( mJobs.front->info[memory_alloc]);
            pcbptr process = dequeue(&mJobs);
            process->memory = allocatedMem;
            ReqRes_alloc(process, process->info[num_printers], process->info[num_scanners],
...
bool ReqRes_check(int printers, int scanners, int modems, int cds) {
...
bool ReqRes_alloc (pcbptr process, int printers, int scanners, int modems, int cds) {
    if (!ReqRes_check(printers, scanners, modems, cds))
        return false;
